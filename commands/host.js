...
module.exports = {
    ...
    init: function(options){
        ...
        else if(_.isNull(options.host)){
            process.stdout.write("Host id required!");
            process.exit(1);
        }
        else
    ...
    options: {
        subcommand: {
            position: 1,
            help: "host subcommand [list, show, edit, delete]",
            required: true,
            choices: ["list", "show", "edit", "delete"]
        },
            ...
            if(err){
                ...
                if(response.body.mode == "follower"){
                    ...
                    _.each(response.body.containers, function(container){
                        ...
                        console.log(sprintf("%-20s %-100s", "", [container.id, " [", container.application, "]"].join("")));
                    });
